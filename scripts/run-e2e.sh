...
# Allow polygon adapter to be started in fixture mode for deterministic E2E runs
ENABLE_POLYGON_ADAPTER=${ENABLE_POLYGON_ADAPTER:-true}
POLYGON_PROFILE_ARGS=()
if [[ "$ENABLE_POLYGON_ADAPTER" == "true" ]]; then
  echo "Polygon adapter coverage enabled (fixture mode by default)"
  export POLYGON_RPC_FIXTURE=${POLYGON_RPC_FIXTURE:-true}
  export POLYGON_PRIVATE_KEY=${POLYGON_PRIVATE_KEY:-dev-key-change-in-production}
  POLYGON_PROFILE_ARGS=(--profile polygon)
else
  echo "Polygon adapter disabled for this E2E run"
fi
...
cleanup() {
  ...
  "${COMPOSE_CMD[@]}" "${POLYGON_PROFILE_ARGS[@]}" rm -f -s -v e2e >/dev/null 2>&1 || true
  # Bring down the compose stack and remove orphans so the network can be cleaned up
  "${COMPOSE_CMD[@]}" "${POLYGON_PROFILE_ARGS[@]}" down --remove-orphans || true
}
...
"${COMPOSE_CMD[@]}" --profile e2e "${POLYGON_PROFILE_ARGS[@]}" run --rm -e E2E_DEBUG e2e
E2E_EXIT_CODE=$?
set -e
