...
# Extract port from ASPNETCORE_URLS (e.g., "http://+:8080" -> "8080" or "http://0.0.0.0:80" -> "80")
# Default to 8080 if not set or cannot parse
PORT=$(echo "$ASPNETCORE_URLS" | sed -E 's/.*:([0-9]+).*/\1/')
