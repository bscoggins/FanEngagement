...
- **Note:** The `polygon-adapter` service is **not** behind a profile and will start by default with `docker compose up`. This is intentional due to the way blockchain testing is implemented in the upstream testing framework—Polygon can run without requiring a dedicated profile for testing purposes.
